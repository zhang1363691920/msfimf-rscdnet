...
def get_scheduler(optimizer, opt):
    ...
    Parameters:
        optimizer优化器     -- the optimizer of the network
        opt (option class) -- stores all the experiment flags; needs to be a subclass of BaseOptions．　
                              opt.lr_policy is the name of learning rate policy: linear | step | plateau | cosine
                              学习率策略
    For 'linear', we keep the same learning rate for the first <opt.niter> epochs
    and linearly decay the rate to zero over the next <opt.niter_decay> epochs.
    在第一个<opt.niter>时期保持相同的学习率，在接下里的<opt.niter_decay>时期线性衰减速率为0
...
class BaseModel(ABC):
    ...
    def __init__(self, opt):
        ...
        Then, you need to define four lists:
            ...
            定义并初始化优化器。可以为每个网络定义一个优化器。如果两个网络同时更新，则可以使用itertools.chain将它们分组。
            请参阅cycle_gan_model.py为例。
        """
        self.opt = opt
        self.gpu_ids = opt.gpu_ids
        self.isTrain = opt.isTrain
        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")  # get device name: CPU or GPU
    ...
    def train(self):
        """Make model train mode during train time"""
        for name in self.model_names:
            if isinstance(name, str):
                net = getattr(self, 'net' + name)
                net.train()
    ...
    def test(self):
        ...
        该函数将<forward>函数包装在no_grad()中，因此我们不为backprop保存中间步骤
