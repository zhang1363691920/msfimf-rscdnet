...
if __name__ == '__main__':
    ...
    for epoch in range(opt.epoch):
        ...
        for step, data in enumerate(dataset):
            ...
            if total_iters % opt.print_freq == 0:
                ...
                message = '(epoch: %d, iters: %d, time: %.3f, data: %.3f, loss: %f) ' % (epoch, total_iters, t_comp, t_data, loss.item())
            ...
            #print('epoch: %d , iters: %d, time: %.3f'%(epoch, step+1, time.time() - iter_start_time))
    ...
    print_current_result(train_log_name, opt.epoch, {"ave_t_epoch": time_ave}, total_loss/((dataset_size/opt.batch_size)*opt.epoch))
